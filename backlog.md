...
| 2 | **Cholesterol knowledge base** | Structured data layer defining what raises vs. lowers cholesterol (saturated fat, trans fat, dietary cholesterol, fiber, omega-3s, etc.) | 60 | 3 | 90% | 2 | **81** | Powers analysis quality. Must be accurate — this is health-adjacent. **Two different projects hide in this row, decide which before speccing:** (a) the KB feeds the prompt and the model still judges — cheap, keeps today's architecture; (b) the KB *is* the scorer, model only extracts ingredients, code does the arithmetic — that's what `Scoring_KB_Generation_Prompt.md` actually describes, and it's a re-architecture. Deferred 2026-08-17. Temperature 0 removed the determinism argument for (b); what survives is multi-condition scaling, bounded fabrication and auditability. **Blocked on two things:** the untested assumption that the model can reliably decompose a dish into ingredients (~10 cents to settle), and per-scan logging, which doesn't exist — so there is no miss data to grow a table from. |
...
| 19 | **Dish categories (drinks / sides / mains), and don't rank alcohol** | Capture each dish's menu section during OCR and carry it as a `category` field: `drink_alcoholic`, `drink_non_alcoholic`, `side`, `condiment`, `main` (default). Rank within category instead of one flat list. Alcoholic drinks are shown and labelled but **not scored or ranked**. | 60 | 3 | 85% | 2 | **77** | **Credibility bug, not a nicety.** The rubric is saturated-fat-driven, so anything with ~no fat scores near 10 whether or not it's food. On Sean's 2026-08-15 brunch scan the top 4 "Top Picks" were Bottomless Classic Orange 9.5, Bottomless Blood Orange 9.5, Seasonal Fruit 9.5, Half Avocado 9.0 — **the app told someone managing cholesterol that bottomless mimosas were their best choice.** Not one of the top 4 is a meal. Cheap because the signal is free: menus label their own sections and `prompts.ts:38` currently tells OCR to *discard* them. Notes: category is data on the dish, not code branches (CLAUDE.md principle 3); a menu's own SAUCES section is unranked, but a sauce named inside a dish description still counts toward the score (hollandaise in a Benedict is the point); condiments must be shown-but-unranked, never silently dropped, or we reintroduce the EAT-9 disappearance bug; `getTag` awards "Top Pick" on score alone and must become category-aware; eval expected tiers become per-category. Needs a spec before build. |

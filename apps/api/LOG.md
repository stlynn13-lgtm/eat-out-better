...
- **Fixed results screen "double load".** The success path navigated to /results twice: `setResults` flips status to `"complete"`, which triggers processing.tsx's effect (`router.replace("/results")`), and then `useAnalysis` *also* called `router.push("/results")` — mounting a second identical results screen and leaving a stale duplicate in the stack. Removed the push; processing.tsx now solely owns success navigation, matching the error-path convention from EAT-6. Verified bundle + app boot; full scan-flow E2E to be confirmed on build 5.
